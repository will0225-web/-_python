...
def evaluate_model(df, predict_lookahead, TP,ETP, SL, max_open_positions, entry_confidence=None, folder_path=None):
    ...
    # 初始化Signal列 
    X_test['Signal'] = 0  
    # 生成買入訊號，當prob_down > confidence level時，信號為1
    X_test.loc[(X_test['prob_down'] > entry_confidence), 'Signal'] = 1
    # 檢查多空訊號
    ...
    for i in range(0, len(X_test)):
        if open_trades:
            ...
            for trade in open_trades[:]:
                    ...
                    # 計算浮動虧損（相對於進場價的最大下跌幅度）
                    unrealized_loss = (entry_price-high_price)
                    trade['max_unrealized_loss'] = min(trade['max_unrealized_loss'], unrealized_loss)
                    # 使用目前價格來計算新的 TP 和 SL
                    # 計算新的止盈和止損條件
                    new_tp_price = last_entry_price * (1 - TP - ETP)
                    new_sl_price = last_entry_price * (1 - SL)
                    # 確保只在新條件更有利時更新
                    trade['tp_price'] = min(trade.get('tp_price', entry_price * (1 - TP - ETP)), new_tp_price)
                    trade['sl_price'] = min(trade.get('sl_price', entry_price * (1 - SL)), new_sl_price)
                    
                     # 檢查止盈條件  
                    if low_price <= trade['tp_price']:
                        exit_price = trade['tp_price']
                        profit = (entry_price-exit_price) * initial_investment / entry_price - fee * initial_investment
                        ...
                        if not signal_found:
                            ...
                            new_sl_price = trade['sl_price'] * 0.95  # 例如，將止損價格提高 5%
                            # 更新止盈和止損價格
                            trade['tp_price'] = new_tp_price
                            # 更新止盈和止損價格
                            trade['sl_price'] = new_sl_price
                            if low_price < trade['tp_price']:  # 如果有正收益但沒達到止盈
                                exit_price = trade['tp_price'] 
                                profit = (entry_price-exit_price) * initial_investment / entry_price - fee * initial_investment
                                X_test.at[entry_index, 'profit'] += profit
                                X_test.at[entry_index, 'Strategy_Return'] += profit / initial_investment
                            ...
                            elif high_price>= trade['sl_price']:   # 持有期滿的止損檢查
                                exit_price = trade['sl_price']                
                                profit = (entry_price-exit_price) * initial_investment / entry_price - fee * initial_investment
                                X_test.at[entry_index, 'profit'] += profit
                                X_test.at[entry_index, 'Strategy_Return'] += profit / initial_investment
...
# predict_lookaheads = [96,168,240,264,288,312,456,480]
# thresholds = [round(i * 0.005, 3) for i in range(1,11)] 
# combinations = [(96, 0.005), (96, 0.01), (120, 0.005), (120, 0.01), (144, 0.005),  (144, 0.01), (144, 0.015), (168, 0.005), (168, 0.01), (168, 0.015),  (192, 0.005), (192, 0.01), (192, 0.015), (216, 0.005), (216, 0.01),  (216, 0.015), (240, 0.005), (240, 0.01), (240, 0.015), (240, 0.02),  (264, 0.005), (264, 0.01), (264, 0.015), (264, 0.02), (288, 0.005),  (288, 0.01), (288, 0.015), (288, 0.02), (312, 0.005), (312, 0.01),  (312, 0.015), (312, 0.02), (336, 0.005), (336, 0.01), (336, 0.015),  (336, 0.02), (336, 0.025), (360, 0.005), (360, 0.01), (360, 0.015),  (360, 0.02), (360, 0.025), (384, 0.005), (384, 0.01), (384, 0.015),  (384, 0.02), (384, 0.025), (408, 0.005), (408, 0.01), (408, 0.015),  (408, 0.02), (408, 0.025), (432, 0.005), (432, 0.01), (432, 0.015),  (432, 0.02), (432, 0.025), (456, 0.005), (456, 0.01), (456, 0.015),  (456, 0.02), (456, 0.025), (456, 0.03), (480, 0.005), (480, 0.01),  (480, 0.015), (480, 0.02), (480, 0.025), (480, 0.03)]
# combinations =[(96, 0.005), (96, 0.01), (96, 0.015), (120, 0.005), (120, 0.01), (120, 0.015), (120, 0.02), (144, 0.005), (144, 0.01), (144, 0.015), (144, 0.02), (168, 0.005), (168, 0.01), (168, 0.015), (168, 0.02), (192, 0.005), (192, 0.01), (192, 0.015), (192, 0.02), (192, 0.025), (216, 0.005), (216, 0.01), (216, 0.015), (216, 0.02), (216, 0.025), (240, 0.005), (240, 0.01), (240, 0.015), (240, 0.02), (240, 0.025), (264, 0.005), (264, 0.01), (264, 0.015), (264, 0.02), (264, 0.025), (264, 0.03), (288, 0.005), (288, 0.01), (288, 0.015), (288, 0.02), (288, 0.025), (288, 0.03), (312, 0.005), (312, 0.01), (312, 0.015), (312, 0.02), (312, 0.025), (312, 0.03), (336, 0.005), (336, 0.01), (336, 0.015), (336, 0.02), (336, 0.025), (336, 0.03), (360, 0.005), (360, 0.01), (360, 0.015), (360, 0.02), (360, 0.025), (360, 0.03), (384, 0.005), (384, 0.01), (384, 0.015), (384, 0.02), (384, 0.025), (384, 0.03), (408, 0.005), (408, 0.01), (408, 0.015), (408, 0.02), (408, 0.025), (408, 0.03), (432, 0.005), (432, 0.01), (432, 0.015), (432, 0.02), (432, 0.025), (432, 0.03), (432, 0.035), (456, 0.005), (456, 0.01), (456, 0.015), (456, 0.02), (456, 0.025), (456, 0.03), (456, 0.035), (480, 0.005), (480, 0.01), (480, 0.015), (480, 0.02), (480, 0.025), (480, 0.03)]
# #combinations = [(96, 0.005), (96, 0.01), (96, 0.015), (120, 0.005), (120, 0.01), (120, 0.015), (144, 0.005), (144, 0.01), (144, 0.015), (168, 0.005), (168, 0.01), (168, 0.015), (168, 0.02), (192, 0.005), (192, 0.01), (192, 0.015), (192, 0.02), (216, 0.005), (216, 0.01), (216, 0.015), (216, 0.02), (240, 0.005), (240, 0.01), (240, 0.015), (240, 0.02), (240, 0.025), (264, 0.005), (264, 0.01), (264, 0.015), (264, 0.02), (264, 0.025), (288, 0.005), (288, 0.01), (288, 0.015), (288, 0.02), (288, 0.025), (312, 0.005), (312, 0.01), (312, 0.015), (312, 0.02), (312, 0.025), (336, 0.005), (336, 0.01), (336, 0.015), (336, 0.02), (336, 0.025), (360, 0.005), (360, 0.01), (360, 0.015), (360, 0.02), (360, 0.025), (360, 0.03), (384, 0.005), (384, 0.01), (384, 0.015), (384, 0.02), (384, 0.025), (384, 0.03), (408, 0.005), (408, 0.01), (408, 0.015), (408, 0.02), (408, 0.025), (408, 0.03), (432, 0.005), (432, 0.01), (432, 0.015), (432, 0.02), (432, 0.025), (432, 0.03), (456, 0.005), (456, 0.01), (456, 0.015), (456, 0.02), (456, 0.025), (456, 0.03), (456, 0.035), (480, 0.005), (480, 0.01), (480, 0.015), (480, 0.02), (480, 0.025), (480, 0.03), (480, 0.035)]
...
base_file_path = fr'C:\\Users\\User\\Documents\\Futures_策略資料_1107'

# 定義輸出結果檔案的完整路徑
base_folder_path = r'C:\\Users\\User\\Documents\\Futures_策略資料_1107'
sub_folder_name = f'Backtest_result_{name}_min_loss_更改持有期間止盈止損'
# 完整資料夾路徑
...
summary_file_path = os.path.join(full_folder_path, f'A_Summary_{name}_min_loss_更改持有期間止盈止損.csv')
