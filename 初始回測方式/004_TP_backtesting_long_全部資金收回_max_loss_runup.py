...
def evaluate_model(df, predict_lookahead, TP,ETP, SL, max_open_positions, entry_confidence=None, folder_path=None):
    ...
    for i in range(0, len(X_test)):
        if open_trades:
            ...
            average_entry_price = sum(trade['entry_price'] for trade in open_trades) / len(open_trades)
            for trade in open_trades[:]:
                    ...
                    #計算runup_stop_price
                    max_profit = max(0, current_close - entry_price,current_close -last_entry_price)  # 計算最大未實現利潤
                    ...
                    trade['sl_price'] = max(trade.get('sl_price', entry_price * (1 + SL)), runup_stop_price)
                    ...
                    if high_price >= trade['tp_price']:
                        ...
                        if not signal_found:
                            ...
                            # runup_stop_price = last_entry_price * (1 + SL) if adjustment_factor > 1 else last_entry_price * adjustment_factor
                            runup_stop_price = trade['sl_price'] * min(
                            1.05,  # 限制調整係數不超過1
                            1 + (max_profit / (run_up_constant*0.1 * current_hold_count)) if run_up_constant!= 0 else 0 # 動態上調的止損調整係數
                            )
...
name = 'Long_correlatcion_win_rate_0.8_train_1723_test_2324_96_480'
...
sub_folder_name = f'Backtest_result_{name}_max_loss_runup'
# 完整資料夾路徑
...
summary_file_path = os.path.join(full_folder_path, f'A_Summary_max_loss_runup.csv')
