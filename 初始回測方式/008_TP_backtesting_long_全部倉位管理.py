...
def evaluate_model(df, predict_lookahead, TP,ETP, SL, max_open_positions, entry_confidence=None, folder_path=None):
    ...
    # Step 1: 持倉管理檢查 - 檢查是否已達到整體止盈或止損
        ...
        if X_test.at[i, 'Signal'] == 1 and len(open_trades) < max_open_positions:
    ...
    # Step 3: 更新所有持倉的止盈和止損價格
        if open_trades:
            ...
            if len(open_trades) == 1 and (i - open_trades[0]['entry_index']) >= predict_lookahead:
                # 將止盈設置得更緊，增加止盈比例
                global_tp_price = open_trades[0]['entry_price'] * (1 + TP)
                # 將止損設置得更緊，減少止損幅度（使止損價格更高）
                global_sl_price = open_trades[0]['entry_price'] * (1 + SL) * 1.05
            # 每480根K線記錄一次利潤與回報
        if i % 480 == 0 and i > 0:
...
# 定義輸出結果檔案的完整路徑
base_folder_path = r'C:\\Users\\User\\Documents\\Backtesting_修改出場邏輯後\\Long_Short_Model_添加Volume_log\\Long_Short_model_all_position'
