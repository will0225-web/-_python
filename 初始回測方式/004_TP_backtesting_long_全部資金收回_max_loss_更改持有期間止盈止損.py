...
import pickle
from math import *
def check_signal(i, lookback, data):
    for j in range(i - lookback, i):
        if j >= 0 and data.at[j, 'Signal'] == 1:
            return True
    return False
def evaluate_model(df, predict_lookahead, TP,ETP, SL, max_open_positions, entry_confidence=None, folder_path=None):
    ...
    X_test['Max_Unrealized_Loss'] = 0.0  # 初始化最大浮動虧損列，初始值設為 0.0
    # 初始化計算變數
    ...
    winning_amounts = []
    losing_amounts = []
    max_unrealized_losses = []  # 用於存儲每筆交易的最大浮動虧損
    ...
    for i in range(0, len(X_test)):
        ...
        if X_test.at[i, 'Signal'] == 1 :
            ...
            if open_trades:
                for trade in open_trades[:]:
                    ...
                    # 使用目前價格來計算新的 TP 和 SL
                    # 計算新的止盈和止損條件
                    new_tp_price = entry_price* (1 + TP + ETP)
                    new_sl_price = entry_price * (1 + SL)
                    # 確保只在新條件更有利時更新
                    if global_tp_price is None or new_tp_price  > global_tp_price:
                        global_tp_price = new_tp_price  # 更新為更高的止盈價，以便更早止盈
                    if global_sl_price is None or new_sl_price  > global_sl_price:
                        global_sl_price = new_sl_price
                        # 拿開倉第一根的話profit目前看起來比較好
                        # global_sl_price = sl_price  # 更新為更高的止損價，以便降低風險

                    trade_log.append(f'更新止盈止損at index {i}, {entry_index}:TP: {global_tp_price},SL: {global_sl_price }')
                        # 檢查止盈條件 

                    if global_sl_price is not None and low_price <= global_sl_price:
                        exit_price = global_sl_price
                        profit = (global_sl_price - entry_price) * initial_investment / entry_price - fee * initial_investment
                        ...
                        X_test.at[entry_index, 'Max_Unrealized_Loss'] = max_unrealized_loss  # 將最大浮動虧損存入相應的進場索引
                        max_unrealized_losses.append(max_unrealized_loss)  # 將最大浮動虧損添加到列表中
                        global_tp_price, global_sl_price = None, None
                        open_trades.clear()
                        trade_log.append(f'平倉{entry_index}: 止損 at index {i}, Profit: {profit}, Max Unrealized Loss: {max_unrealized_loss}')
                        is_close_postion = True
                        continue
                    elif global_tp_price is not None and high_price >= global_tp_price:
                        ...
                        open_trades.clear()
                        # 記錄交易訊息
                        trade_log.append(f'平倉{entry_index}: 止盈 at index {i}, Profit: {profit}, Max Unrealized Loss: {max_unrealized_loss}')
                        # 更新連續虧損計數
                        is_close_postion = True
                        continue
                    elif holding_days >= predict_lookahead:
                        signal_found = False
                        signal_found = check_signal(i, predict_lookahead, X_test)
                        if not signal_found:
                            new_tp_price = entry_price * (1 + TP)  # 捨去 ETP，只使用原本的止盈條件
                            new_sl_price = trade['sl_price'] * 1.05  # 例如，將止損價格提高 5%
                            # 更新止盈和止損價格
                            global_tp_price= new_tp_price
                            # 更新止盈和止損價格
                            global_sl_price= new_sl_price
                            if global_tp_price is not None and high_price > global_tp_price: # 如果有正收益但沒達到止盈
                                exit_price = global_tp_price
                                profit = (high_price- entry_price) * initial_investment / entry_price - fee * initial_investment
                                X_test.at[entry_index, 'profit'] += profit
                                X_test.at[entry_index, 'Strategy_Return'] += profit / initial_investment
                                X_test.at[entry_index, 'Holding_Period'] = holding_days
                                X_test.at[entry_index, 'Exit_Index'] = i
                                ...
                                open_trades.clear()
                                # 記錄交易訊息和最大浮動虧損
                                trade_log.append(f'平倉{entry_index}: 持有期結束獲利 at index {i}, Profit: {profit}, Max Unrealized Loss: {max_unrealized_loss}')
                                is_close_postion = True

                            elif global_sl_price is not None and low_price <= global_sl_price:   # 持有期滿的止損檢查
                                exit_price = global_sl_price               
                                profit = (low_price- entry_price) * initial_investment / entry_price - fee * initial_investment
                                X_test.at[entry_index, 'profit'] += profit
                                X_test.at[entry_index, 'Strategy_Return'] += profit / initial_investment
                                X_test.at[entry_index, 'Holding_Period'] = holding_days
                                X_test.at[entry_index, 'Exit_Index'] = i
                                ...
                                global_tp_price, global_sl_price = None, None
                                open_trades.clear()
                                trade_log.append(f'平倉{entry_index}: 持有期結束於止損 index {i}, Profit: {profit}')
                                is_close_postion = True
                        continue

            if len(open_trades) < max_open_positions and not is_close_postion:
                # 開新倉位
                ...
                open_trades.append({
                    ...
                    'max_loss': 0,  # 初始化最大損失
                    'max_unrealized_loss': 0  # 初始化最大浮動虧損
                })
                # 更新總投資金額
                ...
                trade_log.append(f'開新倉: at index {i}, Entry Price: {entry_price},tp_price:{tp_price},SL_price:{sl_price}')        
        ...
        if i % 480 == 0 and i > 0:
                ...
                if weekly_signal_count == 0:
                    ...
                    no_trade_streak = 0
    if max_unrealized_losses:
        average_max_unrealized_loss = np.mean(max_unrealized_losses)
    else:
        average_max_unrealized_loss = 0  # 如果沒有任何交易，設置為 0

    print(f"平均最大浮動虧損: {average_max_unrealized_loss:.5f}")
    # 計算贏單週數與輸單週數
    ...
    return total_profit, num_wins, num_losses, strategy_sharpe, sharpe_ratio, Strategy_max_drawdown, Commom_Ratio, total_return, max_loss_per_trade, max_consecutive_losses, winning_weeks, losing_weeks, average_winning_amount, average_losing_amount, max_no_trade_weeks, no_trade_weeks, average_hold_count,average_max_unrealized_loss
...
# combinations = [ (120, 0.01),(144, 0.01), (144, 0.015), (168, 0.01), (168, 0.015), (192, 0.01)]
...
# 定義輸出結果檔案的完整路徑
base_folder_path = r'C:\\Users\\User\\Documents\\Backtesting_修改出場邏輯後\\Long_Short_Model_添加Volume_log'
sub_folder_name = f'Backtest_result_{name}_max_loss_更改持有期間止盈止損'
# 完整資料夾路徑
...
summary_file_path = os.path.join(full_folder_path, f'A_Summary_max_loss_更改持有期間止盈止損.csv')
...
with open(summary_file_path,mode='w',newline='') as file:
    ...
    # for predict_lookahead, threshold,ETP_thresholds, stop_loss, max_open_position in combinations:
                            ...
                            writer.writerow([ predict_lookahead, threshold, ETP_threshold, stop_loss, max_open_position, total_profit, num_wins, num_losses, 
                                            ...
                                            winning_weeks, losing_weeks, average_winning_amount, average_losing_amount, max_no_trade_weeks, no_trade_weeks, average_hold_count, average_max_unrealized_loss])
