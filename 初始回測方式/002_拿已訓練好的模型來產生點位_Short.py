...
for predict_lookahead  in range(120,121,24):
        for TP  in thresholds:
            entry_confidence = 0.8
            folder_path = r'C:\\Users\\User\\Documents\\Save_Model\\Long_Short_Model_添加Volume_log\\Save_model_Short_correlatcion_win_rate_0.8_train_1723_test_2324_96_480'
            model_file_path = os.path.join(folder_path , f'xgb_model_{predict_lookahead}_{TP}_down.pkl')

            with open(model_file_path, 'rb') as f:
                model_down= pickle.load(f)
            scaler_file_path = os.path.join(folder_path , f'scaler_{predict_lookahead}_{TP}_down.pkl')
            ...
            new_folder_path = r'C:\\Users\\User\\Documents\\Futures_策略資料_1108'
            new_scaler_file_path = os.path.join(new_folder_path , f'new_scaler_{predict_lookahead}_{TP}_down.pkl')
            ...
            file_path = os.path.join(
                r'C:\\Users\\User\\Documents\\Futures_策略資料_1108',
                f'New_Futures_data_1108.csv'
                    )
            ...
            # c = Result_df_2[Result_df_2['prob_down'] > entry_confidence].shape[0]
            # d = Result_df_2[(Result_df_2['prob_down'] > entry_confidence) & (Result_df_2['True_down_label'] == 1)].shape[0]
            ...
            # 完整的檔案路徑
            full_file_path_down = os.path.join(new_folder_path, file_name_down)
            # 儲存 DataFrame 為 CSV
            Result_df_2.to_csv(full_file_path_down, index=False)
            ...
            plt.title(f'Price vs. Time with Correct and False Predictions (prob_down > {entry_confidence})')
            # Adding legend
            plt.legend()
            # Display the plot
...
# })
# # 生成 CSV 檔案的名稱
# file_name_up = f'Long_Matrix_test_{predict_lookahead}_{TP}_up_1017.csv'    
# # 完整的檔案路徑
# full_file_path_up = os.path.join(folder_path, file_name_up)
# Result_df_2.to_csv(full_file_path_up, index=False)
# file_name_up = f'Scaler_{predict_lookahead}_{TP}_up_1017.csv'    
# full_file_path_up = os.path.join(folder_path, file_name_up)
# X_test_df.to_csv(full_file_path_up, index=False)
...
# plt.xticks(rotation=45)
# plt.tight_layout()
# return_image_path = os.path.join(folder_path, f'Return_{predict_lookahead}_{TP}_1017.jpg')
# plt.savefig(return_image_path, dpi=150)  # 設置 dpi 來提高圖像解析度