...
def evaluate_model(df, predict_lookahead, TP,ETP, SL, max_open_positions, entry_confidence=None, folder_path=None):
    ...
    winning_amounts = []
    losing_amounts = []
    run_up_constant = 1000  # 控制止損價格收緊速度的常數
    # 遍歷所有K線數據
    for i in range(0, len(X_test)):
        ...
        if open_trades:
            ...
            for trade in open_trades[:]:
                    ...
                    max_profit = max(0, entry_price- current_close, last_entry_price- current_close)  # 計算最大未實現利潤
                    current_hold_count = holding_days  # 使用持有天數計算當前持倉時間
                    adjustment_factor = (1 - SL) * (1- (max_profit / (run_up_constant * current_hold_count))) if run_up_constant!= 0 else  (1 - SL)
                    runup_stop_price = last_entry_price * (1 - SL) if adjustment_factor < 1 else last_entry_price * adjustment_factor
                    # 確保只在新條件更有利時更新
                    trade['tp_price'] = min(trade.get('tp_price', entry_price * (1 - TP - ETP)), new_tp_price)
                    trade['sl_price'] = min(trade.get('sl_price', entry_price * (1 - SL)), runup_stop_price)
                    ...
                    if low_price <= trade['tp_price']:
                        ...
                        if not signal_found:
                            ...
                            # adjustment_factor = (1 - SL)*(1- (max_profit / (run_up_constant *0.1* current_hold_count)))
                            # runup_stop_price = last_entry_price * (1 - SL) if adjustment_factor < 1 else last_entry_price * adjustment_factor
                            # # 根據當前價格和 SL 調整移動止損
                            runup_stop_price = trade['sl_price'] * max(
                            0.95,  # 限制調整係數不超過1
                            1 - ((max_profit / (run_up_constant * current_hold_count)) if run_up_constant!= 0 else 0) # 動態上調的止損調整係數
                            )
                            new_sl_price = runup_stop_price
                            # 更新止盈和止損價格
...
combinations = [ (96, 0.005), (96, 0.01),(120, 0.01),(120,0.015),(144, 0.01), (144, 0.015), (168, 0.01), (168, 0.015), (192, 0.01),(192,0.015)]
#
...
name='Short_correlatcion_win_rate_0.8_train_1723_test_2324_96_480'
...
# 定義輸出結果檔案的完整路徑
base_folder_path = r'C:\\Users\\User\\Documents\\Backtesting_修改出場邏輯後\\Long_Short_Model_添加Volume_log\\Long_Short_model'
sub_folder_name = f'Backtest_result_{name}_min_loss_run_up'
# 完整資料夾路徑
...
summary_file_path = os.path.join(full_folder_path, f'A_Summary_min_loss_run_up.csv')
