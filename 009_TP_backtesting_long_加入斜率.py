...
def evaluate_model(df, predict_lookahead, lookback_period, lookback_threshold, TP,ETP, SL, max_open_positions, entry_confidence=None, folder_path=None):
    ...
    X_test.loc[(X_test['prob_up'] > entry_confidence), 'Model_Signal'] = 1
    X_test['Signal'] = 0 
    print(X_test)
    ...
    for i in range(0, len(X_test)):
        ...
        if open_trades:
            ...
            if len(open_trades) <  max_open_positions and (i - open_trades[len(open_trades)-1]['entry_index']) >= predict_lookahead:
                ...
                if signal_found == False and adjusted_flag_partial == False:    
                    ...
                    global_sl_price = global_sl_price*1.05 # 將全局止損價格提高 2%，使得更早觸發止損
        ...
        if global_tp_price is not None and current_high >= global_tp_price:  
            # 整體止盈條件達成，平掉所有持倉
            for trade in open_trades:
                entry_price = trade['entry_price']
                profit = (current_high- entry_price) * initial_investment / entry_price - fee * initial_investment 
        ...
        elif global_sl_price is not None and current_low <= global_sl_price:
            # 整體止損條件達成，平掉所有持倉
            for trade in open_trades:
                entry_price = trade['entry_price']
                loss = (current_low- entry_price) * initial_investment / entry_price - fee * initial_investment
                holding_days = i - trade['entry_index']
                # 更新 DataFrame 結果
...
name = 'Long_correlatcion_win_rate_0.8_train_1723_test_2324_96_480_1031'
