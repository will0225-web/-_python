...
def evaluate_model(df, predict_lookahead, TP,ETP, SL, max_open_positions, entry_confidence=None, folder_path=None):
    ...
    X_test['slope'] = add_weighted_slope_to_df(df, 24)
    print(X_test)
    lookback = 20
    ...
    # )
    X_test['slope_signal']  =   X_test['slope'].apply(lambda x:1 if x>1.2 else 0)
    print(X_test)
    # 初始化Signal列 
...
####開始進行回測數據紀錄和分析
    ...
    return_image_path = os.path.join(folder_path, f'Return_{predict_lookahead}__{TP}_{ETP}_{SL}_{max_open_positions}.jpg')
...
sub_folder_name = f'Backtest_result_{name}_斜率'
# 完整資料夾路徑
...
summary_file_path = os.path.join(full_folder_path, f'A_Summary.csv')
