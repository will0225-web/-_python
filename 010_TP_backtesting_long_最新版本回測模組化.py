...
def evaluate_model(df, predict_lookahead, lookback_period, lookback_threshold, TP,ETP, SL, max_open_positions, entry_confidence=None, folder_path=None):
    ...
    lookback = 20
    X_test['Head_and_Shoulders'] = X_test['Close'].rolling(window=lookback).apply(
    lambda x: detect_head_and_shoulders(x, lookback=lookback), raw=True
    )
    print(X_test)
    # 初始化Signal列 
    ...
    X_test['Signal'] = 0 
    print(X_test)
    X_test['Signal'] = np.where((X_test['Model_Signal'] == 1) , 1, 0)
    ...
    for i in range(0, len(X_test)):
        ...
        if X_test.at[i, 'Signal'] == 1 :
            ...
            if i >= lookback_period:
                max_price = X_test['Close'][i - lookback_period:i].max()
                # 設置下行趨勢條件，例如當前價格低於近期最高價的 98% 時
                downward_trend = current_close < max_price * lookback_threshold
                #min_price_ = X_test['Close'][i - lookback_period:i].max()
                #upward_trend = current_close > min_price_* lookback_threshold
           
                if not downward_trend and len(open_trades) < max_open_positions:
        ...
        if global_tp_price is not None and current_high >= global_tp_price:  
            ...
            for trade in open_trades:
                ...
                profit = (global_tp_price- entry_price) * initial_investment / entry_price - fee * initial_investment 
            ...
            for trade in open_trades:
                entry_price = trade['entry_price']
                loss = (global_sl_price- entry_price) * initial_investment / entry_price - fee * initial_investment
                holding_days = i - trade['entry_index']
                # 更新 DataFrame 結果
...
lookback_periods = [12,24,48,96,120]
lookback_thresholds = [0.9,0.95,0.97,0.98,0.99]
ETP_thresholds = [0,0.01]
...
# 定義輸出結果檔案的完整路徑
base_folder_path = r'C:\\Users\\User\\Documents\\Backtesting_修改出場邏輯後\\Long_Short_Model_添加Volume_log\\Long_Short_model_篩掉反向單'
sub_folder_name = f'Backtest_result_{name}_篩掉反向單'
# 完整資料夾路徑
...
summary_file_path = os.path.join(full_folder_path, f'A_Summary_篩掉反向單.csv')
