...
def evaluate_model(predict_lookahead, TP, entry_confidence, base_folder_path=None,sub_folder_name=None):    
    ...
    r'C:\\Users\\User\\Documents\\Spot策略資料_1019',
    ...
    # 計算每個類別的權重
    #class_weights = compute_class_weight('balanced', classes=np.unique(y_up_train), y=y_up_train)
    ...
    # train_start_time = '2017-08-27 22:00:00'
    # train_end_time = '2020-10-01 00:00:00'
    # # train_start_time = '2020-10-01 00:00:00'
    # # train_end_time = '2023-01-01 00:00:00'  # 訓練資料的結束時間
    # test_start_time = '2020-10-01 00:00:00'
    # test_end_time = '2023-01-01 00:00:00'    # 測試資料的結束時間，驗證資料從這之後開始
    # # #分割資料集，基於時間範圍
    ...
    def scaler(X_train,X_test):
        ...
        # 保存縮放器
        with open(os.path.join(full_folder_path,f'scaler_{predict_lookahead}_{TP}_up.pkl'), 'wb') as f:
    ...
    def find_best_para(threshold):
        ...
        for params in ParameterGrid(param_grid):
            model_up = xgb.XGBClassifier(tree_method="hist", device="cuda",n_estimators=params['n_estimators'],
                                    max_depth=params['max_depth'],
                                    learning_rate=params['learning_rate'],reg_alpha = params['alpha'], gamma = params['gamma'],reg_lambda=params['reg_lambda'],
                                    random_state=42)
            model_up.fit(X_valid,y_up, verbose=True)
            ...
            c = Result_df_2[Result_df_2['prob_up'] > threshold].shape[0]
            d = Result_df_2[(Result_df_2['prob_up'] > threshold) & (Result_df_2['True_up_label'] == 1)].shape[0]
            ...
            trade_log.append(f"Params down: {params},Avg Win Rate: {win_rate_up:.4f},F1 score:{f1_score_up_class_1:.4f}") 

            ####找最大勝率
            if win_rate_up >= best_score_up:
                best_score_up = win_rate_up 
                best_params_up = params
                best_result_df_up=Result_df_2
                best_y_pred_up = y_pred_up
                best_model_up =model_up

            print(f"df_data_{predict_lookahead}_{TP}_Params: {params}, F1 up : {f1_score_up_class_1:.4f}")
            print(f"Best Params up: {best_params_up}, Best Avg Win Rate: {best_score_up:.4f}")

        return best_model_up, best_score_up, best_params_up, best_result_df_up, best_y_pred_up
    
    best_model_up,best_score_up,best_params_up,best_result_df_up,best_y_pred_up =find_best_para(entry_confidence)
    
    param_log.append(f'df_data_{predict_lookahead}_{TP}_Best Params: {best_params_up}, Best Avg Win Rate: {best_score_up:.4f}')
    
    # 生成 CSV 檔案的名稱
    file_name_up = f'Long_Short_Matrix_test_{predict_lookahead}_{TP}_up.csv'    
    # 完整的檔案路徑
    full_file_path_up = os.path.join(full_folder_path, file_name_up)
    # 儲存 DataFrame 為 CSV
    best_result_df_up.to_csv(full_file_path_up, index=False)
    ...
    plt.scatter(best_result_df_up[(best_result_df_up['Predict_up_label'] == 1) & 
                                    (best_result_df_up['True_up_label'] == 1) & 
    ...
    with open(os.path.join(full_folder_path,f'xgb_model_{predict_lookahead}_{TP}_up.pkl'), 'wb') as f:
        pickle.dump(best_model_up, f)
    #with open(os.path.join(folder_path,f'xgb_model_{predict_lookahead}_{TP}_down.pkl'), 'wb') as f:
    #    pickle.dump(best_model_down, f)
    
    
    y_new_pred_proba_up = best_model_up.predict_proba(X_valid_2)
    prob_up_class_1 = y_new_pred_proba_up[:, 1]
    df_2['prob_up'] = prob_up_class_1
    thresholds_up = [0.95,0.9, 0.8, 0.7,0.6,0.5]
    results_up = {"Threshold": [], "Win Rate": [],"Win Count":[],"Total Count":[]}
    #print(df_down)
    for threshold in thresholds_up:
        c = best_result_df_up[best_result_df_up['prob_up'] > threshold].shape[0]
        d = best_result_df_up[(best_result_df_up['prob_up'] > threshold) & (best_result_df_up['True_up_label'] == 1)].shape[0]
        win_rate_up = d/c if c != 0 else 0
        results_up["Threshold"].append(f"Prob_up_over_{threshold}")
        results_up["Win Rate"].append(win_rate_up)
        results_up["Win Count"].append(d)
        results_up["Total Count"].append(c)
        print(f'Prob_up_over_{threshold}_win_rate[{win_rate_up}]')
    df_up = pd.DataFrame(results_up)
    #print(df_up)
    accuracy_up = accuracy_score(y_test_up, best_y_pred_up)
    print(f'Accuracy_up: {accuracy_up:.2f}')
    #print(f'Accuracy_down: {accuracy_down:.2f}')
    auc_score_up = roc_auc_score(y_test_up, prob_up_class_1 )
    ...
    # 計算分類報告並轉換為DataFrame
    report_up = classification_report(y_test_up, best_y_pred_up, output_dict=True)
    df_report_up = pd.DataFrame(report_up).transpose()
    print(df_report_up)
    f1_score_up_class_1 = report_up['1']['f1-score']
    #print(classification_report(y_test_down, best_y_pred_down))
# 創建一個ExcelWriter物件
    return f1_score_up_class_1, df_up, df_report_up, accuracy_up, auc_score_up


import csv
thresholds = [round(i * 0.005, 3) for i in range(1,7)] 
...
name = 'Long_correlatcion_win_rate_0.8_train_1723_test_2324_96_480_添加volume_log'
# 定義輸出結果檔案的完整路徑
...
with open(summary_file_path, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['','','','','','predict_lookahead','thresholds','Accuracy_up','auc_score_up','f1_score_up_class_1'])
    for lookahead in range(96,481,24):
        for d in thresholds:
            # 主資料夾路徑
            base_folder_path = r'C:\\Users\\User\\Documents\\Save_Model'        
            # 使用當前時間生成子資料夾名稱，或根據其他參數自定義名稱
            sub_folder_name = f'Save_model_{name}'
            f1_score_up_class_1,df_up,df_report_up,accuracy_up, auc_score_up = evaluate_model(lookahead,d,0.8,base_folder_path=base_folder_path,sub_folder_name=sub_folder_name)
            writer.writerow(['','','','','',lookahead,d,accuracy_up, auc_score_up ,f1_score_up_class_1 ])
            writer.writerow([f'評估結果_{lookahead}_{d}'])
            # 空行作為分隔
            # 寫入 df_down 的標題和內容
            writer.writerow(['','df_up Results'])
            df_up.insert(0,'','') 
            df_up.to_csv(file, index=False)
            writer.writerow(['','Classification Report Up'])
            df_report_up.insert(0,'', '') 
            df_report_up.to_csv(file,index=False) 
