...
# 頭肩頂形態檢測 (針對每一個位置進行判斷)
# 偵測頭肩頂和倒頭肩頂形態
# 偵測頭肩頂和倒頭肩頂形態，考慮lookback範圍
# 滾動檢測頭肩頂和倒頭肩頂形態
def detect_head_and_shoulders(data, lookback=20):
    # 僅考慮最後 lookback 根 K 線
    window_data = data[-lookback:]
    pattern = 0  # 預設無形態

    # 找出峰值（頭肩頂）和谷值（倒頭肩頂）
    peaks, _ = find_peaks(window_data)       # 局部高點
    troughs, _ = find_peaks(-window_data)    # 局部低點（倒頭肩頂）

    # 檢查頭肩頂形態
    if len(peaks) >= 3:  # 至少要有三個峰值
        left_shoulder = window_data[peaks[-3]]
        head = window_data[peaks[-2]]
        right_shoulder = window_data[peaks[-1]]
    ...
    # 檢查倒頭肩頂形態
    if len(troughs) >= 3:  # 至少要有三個谷值
        left_shoulder = window_data[troughs[-3]]
        head = window_data[troughs[-2]]
        right_shoulder = window_data[troughs[-1]]

        # 符合倒頭肩頂條件
        if head < left_shoulder and head < right_shoulder and abs(left_shoulder - right_shoulder) < 0.05 * abs(head):
            pattern = 1  # 做多信號（倒頭肩頂）

    return pattern
def add_weighted_slope_to_df(df, lookback, alpha=1):
    """
    在给定的DataFrame中添加带权重的slope列，用于每个位置的lookback斜率.

    参数:
    - df: 包含K线数据的DataFrame，必须包含 'Close' 列
    - lookback: 回溯的K线数量，用于计算趋势的斜率
    - alpha: 权重递减因子，越接近1对较新的数据权重越大

    返回:
    - df: 带有新列 'weighted_slope' 的DataFrame
    """
    weighted_slopes = []

    for i in range(len(df)):
        if i < lookback - 1:
            weighted_slopes.append(np.nan)  # 在lookback不足时填充NaN
        else:
            # 获取回溯窗口内的收盘价
            close_prices = df['Close'].iloc[i - lookback + 1:i + 1].values
            x = np.arange(lookback)  # 创建时间序列

            # 生成权重，使得越新的数据权重越高
            weights = np.array([alpha**(lookback - j - 1) for j in range(lookback)])
            weighted_mean_x = np.average(x, weights=weights)
            weighted_mean_y = np.average(close_prices, weights=weights)

            # 计算加权斜率
            numerator = np.sum(weights * (x - weighted_mean_x) * (close_prices - weighted_mean_y))
            denominator = np.sum(weights * (x - weighted_mean_x) ** 2)
            weighted_slope = numerator / denominator
            weighted_slopes.append(weighted_slope)
...
def evaluate_model(df, predict_lookahead, lookback_period, lookback_threshold, TP,ETP, SL, max_open_positions, entry_confidence=None, folder_path=None):
    ###載入回測資料
    X_test = df[['Close','High','Low','prob_down','datetime']]
    X_test['slope'] = add_weighted_slope_to_df(df, lookback_period)
    X_test['slope_signal']  =  X_test['slope'].apply(lambda x:1 if x < lookback_threshold else 0)

    # 初始化Signal列 
    X_test['Model_Signal'] = 0  
    # 生成買入訊號，當prob_up > entry confidence且信號為1
    X_test.loc[(X_test['prob_down'] > entry_confidence), 'Model_Signal'] = 1
    X_test['Signal'] = 0 
    print(X_test)
    X_test['Signal'] = np.where(((X_test['Model_Signal'] == 1)  & (X_test['slope_signal'] == 1) ) , 1, 0)
    ...
    total_profit = 0  # 總利潤
    total_loss = 0  # 用於記錄總虧損
    net_profit = 0  # 用於記錄總利潤（包含贏和虧）
    ...
    winning_amounts = []
    losing_amounts = []
    global_tp_price = None
    global_sl_price = None
    # 初始化標誌位
    adjusted_flag_partial = False  # 用於未滿倉的縮緊操作
    adjusted_flag_full = False     # 用於滿倉的縮緊操作
    new_trade_opened_flag = False  # 用於指示是否有新倉位被開立
    # 遍歷所有K線數據
    for i in range(0, len(X_test)):
        current_close = X_test.at[i, 'Close']
        current_high = X_test.at[i, 'High']
        current_low = X_test.at[i, 'Low']

        # Step 2: 新訊號檢查 - 檢查是否有新的開倉信號
        if X_test.at[i, 'Signal'] == 1 :
            ...
            if len(open_trades) < max_open_positions:
                        ...
                        trade_count += 1 
                        trade_log.append(f'開新倉 at index {i},entry_price: {entry_price}, tp_price: {tp_price},sl_price = {sl_price} ')
                        new_trade_opened_flag = True  # 標誌新倉位已開立
        # Step 3: 更新所有持倉的止盈和止損價格
        if open_trades:
            if len(open_trades) <= max_open_positions:
                if(adjusted_flag_partial == False and adjusted_flag_full == False) or new_trade_opened_flag:
                    for trade in open_trades:
                        # 根據每個持倉的entry_price計算新的止盈和止損價格
                        tp_price = trade['entry_price'] * (1 - TP -ETP)
                        sl_price = trade['entry_price'] * (1 - SL)
                        # 更新全局的止盈和止損價格
                        if global_tp_price is None or tp_price < global_tp_price:
                            global_tp_price = tp_price  # 更新為更高的止盈價，以便更早止盈
                        if global_sl_price is None or sl_price > global_sl_price:
                            global_sl_price = sl_price  # 更新為更高的止損價，以便降低風險
                    trade_log.append(f'第一輪更新止盈和止損價格 at index {i}, tp_price: {global_tp_price},sl_price = {global_sl_price} ')
                    adjusted_flag_partial = False  # 設置部分縮緊標誌位
                    new_trade_opened_flag = False  # 重置新開倉位標誌
                # 當到達最後一個持倉且持有期超過predict_lookahead時，調整止盈止損條件
            if len(open_trades) <  max_open_positions and (i - open_trades[len(open_trades)-1]['entry_index']) >= predict_lookahead:
                signal_found = any(X_test.at[j, 'Signal'] == 1 for j in range(i - predict_lookahead, i) if j >= 0)   
                if signal_found == False and adjusted_flag_partial == False:    
                    global_tp_price = open_trades[len(open_trades)-1]['entry_price'] *(1-TP) # 將全局止盈價格減少 2%，使得更容易達到止盈
                    # 將止損設置得更緊，減少止損幅度（使止損價格更高）  
                    global_sl_price = global_sl_price*0.95 # 將全局止損價格提高 2%，使得更早觸發止損
                    adjusted_flag_partial = True 
                    signal_found == False
                    trade_log.append(f'最大倉位未到更新止盈和止損價格 at index {i}, tp_price: {global_tp_price},sl_price = {global_sl_price} ')
            
            if len(open_trades) == max_open_positions and (i - open_trades[max_open_positions-1]['entry_index']) >= predict_lookahead:
                # 將止盈設置得更緊，增加止盈比例
                signal_found = any(X_test.at[j, 'Signal'] == 1 for j in range(i - predict_lookahead, i) if j >= 0)   
                if signal_found == False and adjusted_flag_full ==False:
                    global_tp_price = open_trades[max_open_positions-1]['entry_price'] *(1-TP) # 將全局止盈價格減少 2%，使得更容易達到止盈
                    # 將止損設置得更緊，減少止損幅度（使止損價格更高）
                    global_sl_price =global_sl_price *0.95 # 將全局止損價格提高 2%，使得更早觸發止損
                    adjusted_flag_full = True
                    signal_found == False
                    trade_log.append(f'最後一輪更新止盈和止損價格 at index {i}, tp_price: {global_tp_price},sl_price = {global_sl_price} ')
            
            # Step 1: 持倉管理檢查 - 檢查是否已達到整體止盈或止損
        if global_tp_price is not None and current_low <= global_tp_price:  
            # 整體止盈條件達成，平掉所有持倉
            for trade in open_trades:
                entry_price = trade['entry_price']
                profit = (entry_price-global_tp_price) * initial_investment / entry_price - fee * initial_investment 
                holding_days = i - trade['entry_index'] 
                # 更新 DataFrame 結果
                X_test.at[trade['entry_index'], 'profit'] += profit 
                X_test.at[trade['entry_index'], 'Strategy_Return'] += profit / initial_investment 
                X_test.at[trade['entry_index'], 'Holding_Period'] = holding_days 
                X_test.at[trade['entry_index'], 'Exit_Index'] = i 
                total_profit += profit
                net_profit += profit
                #total_investment -= initial_investment
            ...
            # 清空所有持倉
            open_trades.clear()
            trade_log.append(f'平倉: 止盈 at index {i}, global_tp_price : {global_tp_price }')
            # 重置止盈止損條件
            global_tp_price, global_sl_price = None, None
            adjusted_flag_partial = False
            adjusted_flag_full = False
            continue

        elif global_sl_price is not None and current_high >= global_sl_price:
            # 整體止損條件達成，平掉所有持倉
            for trade in open_trades:
                entry_price = trade['entry_price']
                loss = (entry_price-global_sl_price) * initial_investment / entry_price - fee * initial_investment
                holding_days = i - trade['entry_index']
                # 更新 DataFrame 結果
                X_test.at[trade['entry_index'], 'profit'] += loss
                X_test.at[trade['entry_index'], 'Strategy_Return'] += loss / initial_investment
                X_test.at[trade['entry_index'], 'Holding_Period'] = holding_days
                X_test.at[trade['entry_index'], 'Exit_Index'] = i
                total_loss += loss
                net_profit += loss
            ...
            # 清空所有持倉
            open_trades.clear()
            trade_log.append(f'平倉: 止損 at index {i}, global_sl_price: {global_sl_price}')
        ...
        if i % 480 == 0 and i > 0:
                ...
                return_rate = net_profit / total_investment if total_investment > 0 else 0
...
####開始進行回測數據紀錄和分析
    ...
    total_profit = X_test['profit'].sum()
    Final_Return = X_test['Strategy_Return'].sum()
    print(f'Total Profit: {total_profit}, Final Return: {Final_Return}')
    ...
    return_image_path = os.path.join(folder_path, f'Return_{predict_lookahead}_{lookback_period}_{lookback_threshold}_{TP}_{ETP}_{SL}_{max_open_positions}.jpg')
    ...
    plt.legend()
    profit_image_path = os.path.join(folder_path,f'Profit_{predict_lookahead}_{lookback_period}_{lookback_threshold}_{TP}_{ETP}_{SL}_{max_open_positions}.jpg')
    ...
    # plt.figure(figsize=(14,7))
    # plt.plot(X_test['datetime'],X_test['Close'], label='Close Price', color='blue')
    # plt.scatter(X_test[(X_test['Signal']==1)]['datetime'],
    #             X_test[(X_test['Signal']==1)]['Close'],
    #             color='green', label='點位判斷', zorder=5)
    # plt.xlabel('Datetime')
    ...
    # plt.tight_layout()
    # point_image_path = os.path.join(folder_path,f'Point_{predict_lookahead}_{TP}_{ETP}_{SL}_{max_open_positions}.jpg')
    # plt.savefig(point_image_path, dpi=150)  # 設置 dpi 來提高圖像解析度
    # plt.close()
    # plt.show()
    ...
    file_name_up =f'Weekly_profit_invest_{predict_lookahead}_{lookback_period}_{lookback_threshold}_{TP}_{ETP}_{SL}_{max_open_positions}.csv'
    ...
    X_test_up = f'Backtesting_{predict_lookahead}_{lookback_period}_{lookback_threshold}_{TP}_{ETP}_{SL}_{max_open_positions}.csv'
    ...
    summary_df_name_log = f'df_trade_log_{predict_lookahead}_{lookback_period}_{lookback_threshold}_{TP}_{ETP}_{SL}_{max_open_positions}.csv'
...
#combinations = [ (96, 0.005), (96, 0.01), (120, 0.005),(120, 0.01),(144, 0.01), (144, 0.015), (168, 0.01), (168, 0.015), (192, 0.01)]
#combinations =[(144,0.01)]
lookback_periods = [24,48,72,96,120,144,168,192]
lookback_thresholds = [-1,-2,-3,-4,-5]
ETP_thresholds = [0.01]
stop_losses = [-0.02,-0.05,-0.1,-0.2]
max_positions = [10,20] 
 

name = 'Short_correlatcion_win_rate_0.8_train_1723_test_2324_96_480_1031'
...
# 定義輸出結果檔案的完整路徑
base_folder_path = r'C:\\Users\\User\\Documents\\Backtesting_修改出場邏輯後\\Long_Short_Model_添加Volume_log\\Long_Short_model_斜率'
sub_folder_name = f'Backtest_result_{name}_grid_slpoe'
# 完整資料夾路徑
        ...
        for lookback_threshold in lookback_thresholds:
            for predict_lookahead in predict_lookaheads:
                    ...
                    # for predict_lookahead, threshold,ETP_thresholds, stop_loss, max_open_position in combinations:
                                            ...
                                            base_file_path,
                                            f'Short_Matrix_test_{predict_lookahead}_{threshold}_down.csv' )
                                            df= pd.read_csv(file_path)
                                            ###儲存結果資料夾
                                            folder_path = full_folder_path  
                                            
                                            total_profit, num_wins, num_losses, strategy_sharpe, sharpe_ratio, Strategy_max_drawdown, Commom_Ratio, total_return, max_loss_per_trade, max_consecutive_losses, winning_weeks, losing_weeks, average_winning_amount, average_losing_amount, max_no_trade_weeks, no_trade_weeks, average_hold_count= evaluate_model(df,predict_lookahead, lookback_period, lookback_threshold,threshold,ETP_threshold, stop_loss, max_open_position,
                                            entry_confidence=0.8, folder_path=folder_path)
                                            
                                            writer.writerow([ predict_lookahead, lookback_period, lookback_threshold, threshold, ETP_threshold, stop_loss, max_open_position, total_profit, num_wins, num_losses, 
